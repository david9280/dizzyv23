...
echo isset($request->getUrl()) ? $request->getUrl() : NULL;
// >> http://www.foo.com/
```
...
$client->addSubscriber(new LogPlugin(new ClosureLogAdapter(function($m) { echo filter_var($m, FILTER_SANITIZE_STRING); }, $verbosity = LogPlugin::LOG_VERBOSE);
```
...
$client->addSubscriber(new LogPlugin(new ClosureLogAdapter(function($m) { echo filter_var($m, FILTER_SANITIZE_STRING); }, $format = MessageFormatter::DEBUG_FORMAT);
```
