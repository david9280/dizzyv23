...
echo isset($customer) ? $customer : NULL;
```
...
echo isset($curl->getTimeout()) ? $curl->getTimeout() : NULL; // 10
echo isset($curl->getConnectTimeout()) ? $curl->getConnectTimeout() : NULL; // 5
...
echo isset($customer->getLastResponse()->headers['Request-Id']) ? $customer->getLastResponse()->headers['Request-Id'] : NULL;
```
